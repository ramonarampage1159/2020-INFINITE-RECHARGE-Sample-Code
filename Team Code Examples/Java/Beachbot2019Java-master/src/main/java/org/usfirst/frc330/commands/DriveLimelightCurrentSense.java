 ...
 *
 */
public class DriveLimelightCurrentSense extends BBCommand {

    double throttle;
    int thresholdCount = 0;
    int threshold;

    public DriveLimelightCurrentSense(double throttle, double timeout, int threshold) {
    ...
    protected void execute() {
        ...
        if(Robot.pickup.getPickupCurrent() > GrabberConst.currentLimit){
            thresholdCount ++;
        }
        else{
            thresholdCount = 0;
    ...
    protected boolean isFinished() {
        if(this.isTimedOut() || (thresholdCount>threshold)){
            return true;
        }
        else{
            // if(NetworkTableInstance.getDefault().getTable("limelight").getEntry("tv").getDouble(0) > 0.5){
